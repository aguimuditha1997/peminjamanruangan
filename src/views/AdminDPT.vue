      ...
      <div class="p-6 border-t border-white/5">
        <div class="flex items-center gap-3">
          ...
          <div class="flex flex-col">
            ...
            <span class="text-[10px] text-slate-500 uppercase font-bold">DPT Lead</span>
          </div>
        </div>
            ...
            <table class="w-full text-left">
              <thead>
                <tr class="text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-white/5 bg-white/[0.01]">
                  <th class="px-6 py-4">Request Code</th>
              ...
              <tbody class="divide-y divide-white/5">
                ...
                <tr v-else v-for="request in filteredRequests" :key="request.id" class="hover:bg-white/[0.02] transition-colors group text-sm">
                  ...
                  <td class="px-6 py-5">
                    <span :class="[
                      ...
                      (request.status_dpt || '').toLowerCase().startsWith('reject') ? 'bg-red-500/10 text-red-400' : 
                      'bg-amber-500/10 text-amber-400'
                    ]">
                      {{ request.status_dpt || 'pending' }}
                    </span>
                  </td>
                ...
                <tr v-if="!isLoading && filteredRequests.length === 0">
                  <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                    ...
                    No technical requests matching your search.
                  </td>
                </tr>
              </tbody>
            </table>
...
const manageForm = ref({ status_dpt: '', start_time: '', end_time: '', room_id: '' })

const rooms = ref([])
const requests = ref([])

const fetchDPTData = async () => {
  ...
  return requests.value
    .filter(req => {
      ...
      const matchesStatus = filters.value.status === 'all' || 
                           (req.status_dpt || 'pending').toLowerCase() === filters.value.status
...
function openManageModal(request) {
  ...
  // Backend returns start_time/end_time in YYYY-MM-DD HH:mm:ss
  manageForm.value = { 
    status_dpt: (request.status_dpt || 'pending').toLowerCase(), 
    start_time: (request.start_time || '').replace(' ', 'T').substring(0, 16), 
    end_time: (request.end_time || '').replace(' ', 'T').substring(0, 16),
...
const saveChanges = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      status: manageForm.value.status_dpt,
      status_dpt: manageForm.value.status_dpt,
